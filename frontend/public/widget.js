...
(function () {
  ...
  var platformUrl = new URL(script.src, window.location.href);
  if (window.location.protocol === "https:" && platformUrl.protocol !== "https:") {
    console.error("AI Receptionist widget: HTTPS pages require an HTTPS widget URL.");
    return;
  }

  function mount() {
    if (!document.body || document.querySelector("[data-ai-receptionist-widget]")) return;

    var iframe = document.createElement("iframe");
    iframe.src = platformUrl.origin + "/chat/" + encodeURIComponent(agent);
    iframe.title = "AI receptionist";
    iframe.setAttribute("loading", "lazy");
    iframe.setAttribute("referrerpolicy", "no-referrer");
    iframe.setAttribute("data-ai-receptionist-widget", "frame");
    ...
    button.setAttribute("data-ai-receptionist-widget", "button");
