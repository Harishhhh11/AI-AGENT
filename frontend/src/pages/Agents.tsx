...
import { createAgent, deleteAgent, publishAgent, updateAgent } from "../api/agents";
...
export default function Agents() {
  ...
  function openEdit(agent: Agent) {
    ...
    setForm({ name: agent.name, public_slug: agent.public_slug, welcome_message: agent.welcome_message, system_instructions: "", knowledge_item_ids: agent.knowledge_item_ids ?? [] });
    setOpen(true); setError(""); setSuccess("");
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (selectedForEdit) {
        ...
        const knowledgeUpdated = await updateAgentKnowledgeSafely(selectedForEdit.id, form.knowledge_item_ids ?? []);
        setAgents((current) => current.map((agent) => agent.id === updated.id ? { ...knowledgeUpdated, ...updated, knowledge_item_ids: knowledgeUpdated.knowledge_item_ids } : agent));
  ...
  async function togglePublish(agent: Agent) {
    ...
    setSaving(true); setError(""); setSuccess("");
    try {
      const updated = agent.is_published ? await import("../api/agents").then(({ unpublishAgent }) => unpublishAgent(agent.id)) : await publishAgent(agent.id);
...
async function updateAgentKnowledgeSafely(id: number, knowledgeItemIds: number[]): Promise<Agent> {
  const { updateAgentKnowledge, getAgent } = await import("../api/agents");
  await updateAgentKnowledge(id, { knowledge_item_ids: knowledgeItemIds });
  return getAgent(id);
}
