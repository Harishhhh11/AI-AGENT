...
def list_agents(
    ...
    return AgentService(db).get_all(
        current_user.organization_id
    )
...
def get_agent(
    ...
    if not agent:
        raise HTTPException(
            status_code=404,
            detail="AI receptionist not found.",
        )
...
def publish_agent(
    ...
    agent = AgentService(db).set_published(
        agent_id,
        current_user.organization_id,
        True,
    )
...
def get_public_agent(
    ...
    agent = AgentService(db).get_public(
        public_slug
    )
    ...
    if not agent:
        raise HTTPException(
            status_code=404,
            detail="This AI receptionist is unavailable.",
        )
...
@router.post(
    "/public/agents/{public_slug}/chat",
...
async def public_agent_chat(
    ...
    session_id, response = await ChatService(
        db
    ).generate_response(
